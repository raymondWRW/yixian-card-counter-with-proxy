📦
133824 /native_hud/agents/bot_glue3.js
✄
...
// native_hud/agents/bot_glue3.ts
function raw(x) {
  return x && x.content != null ? x.content : "" + x;
}
function findClassFull(full, simple) {
  for (const asm of Il2Cpp.domain.assemblies) {
    try {
      const k = asm.image.class(full);
      if (k)
        return k;
    } catch (_) {
    }
...
function getIType(name) {
  const dict = AD.field("mapType").value.method("get_InnerDictionary").invoke();
  const entries = dict.field("entries").value, count = dict.field("count").value;
  for (let i = 0; i < count; i++) {
    try {
      const e = entries.get(i);
      const k = e.field("key").value;
      if (k != null && raw(k) === name)
        return e.field("value").value;
    } catch (_) {
    }
  }
  return null;
}
function getMethod(typeName, methodName, pc) {
  const it = getIType(typeName);
  if (!it)
    return null;
  const ms = it.method("GetMethods").invoke();
  const n = ms.method("get_Count").invoke();
  for (let i = 0; i < n; i++) {
    const m = ms.method("get_Item").invoke(i);
    if (raw(m.method("get_Name").invoke()) !== methodName)
      continue;
    try {
      if ("" + m.method("get_ParameterCount").invoke() !== "" + pc)
        continue;
    } catch (_) {
    }
    return m;
...
function boxInt(n) {
  const o = Il2Cpp.corlib.class("System.Int32").alloc();
  o.field("m_value").value = n;
  return o;
}
function findCardPanel() {
  for (const inst of Il2Cpp.gc.choose(ITI)) {
    try {
      if (raw(inst.method("get_Type").invoke().method("get_Name").invoke()) === "CardPanel")
        return inst;
    } catch (_) {
    }
  }
...
function invokeNamed(typeName, method, args) {
  const m = getMethod(typeName, method, args.length);
  if (!m)
    throw new Error(`${typeName}.${method}/${args.length} not found`);
...
rpc.exports = {
  loadBot(dll) {
    return Il2Cpp.perform(() => {
      try {
        ...
        const arr = Il2Cpp.array(Il2Cpp.corlib.class("System.Byte"), u8.length);
        for (let i = 0; i < u8.length; i++)
          arr.set(i, u8[i]);
        const ms = Il2Cpp.corlib.class("System.IO.MemoryStream").alloc();
  ...
  ping(typeName) {
    return Il2Cpp.perform(() => {
      try {
        getAD();
        return { ok: true, result: invokeNamed(typeName, "Ping", []) };
      } catch (e) {
        return { ok: false, err: "" + e };
      }
    });
  ...
  call(typeName, method, ints) {
    return Il2Cpp.perform(() => {
      try {
        getAD();
        const cp = findCardPanel();
        if (!cp || cp.isNull())
          return { ok: false, err: "no live CardPanel (\u4E0D\u5728\u6446\u724C\u754C\u9762?)" };
  ...
  callS(typeName, method, ints) {
    return Il2Cpp.perform(() => {
      try {
        getAD();
        const args = (ints || []).map(boxInt);
        return { ok: true, result: invokeNamed(typeName, method, args) };
      } catch (e) {
        return { ok: false, err: "" + e };
      }
    ...
    return Il2Cpp.perform(() => {
      try {
        ...
        for (const it of Il2Cpp.gc.choose(ITI)) {
          try {
            if (raw(it.method("get_Type").invoke().method("get_Name").invoke()) === typeName) {
              inst = it;
              break;
            }
          } catch (_) {
        ...
        if (!inst || inst.isNull())
          return { ok: false, err: "no live " + typeName };
        const m = getMethod(typeName, method, 0);
        if (!m)
          return { ok: false, err: method + " not found on " + typeName };
  ...
  pickAny() {
    return Il2Cpp.perform(() => {
      try {
        getAD();
        const scan = (tn) => {
          for (const it2 of Il2Cpp.gc.choose(ITI)) {
            try {
              if (raw(it2.method("get_Type").invoke().method("get_Name").invoke()) === tn)
                return it2;
            } catch (_) {
            }
          }
        ...
        const inv = (it2, tn, m, args) => {
          const mm = getMethod(tn, m, args.length);
          if (!mm)
            throw new Error(tn + "." + m + " missing");
          return INVOKE.invoke(mm, it2, Il2Cpp.array(OBJCLS, args));
        };
  ...
  sendmsg(typeName, base64) {
    return Il2Cpp.perform(() => {
      try {
        getAD();
        const gm = getMethod("GameClientUtil", "get_client", 0);
        if (!gm)
          return { ok: false, err: "no GameClientUtil.get_client" };
        const gc = INVOKE.invoke(gm, NULL, Il2Cpp.array(OBJCLS, []));
        if (!gc || gc.isNull())
          return { ok: false, err: "client null" };
        const pp = gc.field("m_RoomParser").value;
        if (!pp || pp.isNull())
          return { ok: false, err: "no m_RoomParser" };
        const msg = pp.method("DecodeFromBase64", 2).invoke(Il2Cpp.string(typeName), Il2Cpp.string(base64));
        if (!msg || msg.isNull())
          return { ok: false, err: "decode " + typeName + " failed" };
  ...
  toast(msg) {
    return Il2Cpp.perform(() => {
      try {
        ...
        if (!cls)
          return { ok: false, err: "no HintMessagePanel" };
  ...
  callStr(typeName, method, str) {
    return Il2Cpp.perform(() => {
      try {
        getAD();
        const m = getMethod(typeName, method, 1);
        if (!m)
          return { ok: false, err: typeName + "." + method + " not found" };
