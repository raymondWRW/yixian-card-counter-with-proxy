📦
126273 /autoplay/inject/capture.js
✄
...
// autoplay/inject/capture.ts
function findClass(name) {
  for (const asm of Il2Cpp.domain.assemblies) {
    try {
      const k = asm.image.class(name);
      if (k)
        return k;
    } catch (_) {
    }
...
function s(x) {
  try {
    if (x == null)
      return "";
    return x.content != null ? x.content : "" + x;
  } catch (_) {
    return "";
  }
}
Il2Cpp.perform(() => {
  ...
  if (!pc) {
    console.log("no ProtobufParser");
    return;
  }
  const dec = pc.methods.find((m) => m.name === "DecodeFromBase64" && m.parameterCount === 2 && !m.virtualAddress.isNull());
  if (dec) {
    const orig = new NativeFunction(dec.virtualAddress, "pointer", ["pointer", "pointer", "pointer", "pointer"]);
    dec.implementation = function(typeName, base64) {
      try {
        send({ dir: "in", t: s(typeName), b: s(base64) });
      } catch (_) {
      }
      return new Il2Cpp.Object(orig(this.handle, typeName.handle, base64.handle, dec.handle));
    };
    console.log("hooked DecodeFromBase64 (inbound)");
  }
  const enc = pc.method("EncodeToProtobufData");
  if (enc) {
    const orig = new NativeFunction(enc.virtualAddress, "pointer", ["pointer", "pointer", "pointer"]);
    enc.implementation = function(msg) {
      const retPtr = orig(this.handle, msg.handle, enc.handle);
      try {
        const pd = new Il2Cpp.Object(retPtr);
