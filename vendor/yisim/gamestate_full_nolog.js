...
export class Player {
    ...
    reset() {
        ...
        // R22: turn-start conditional-continuous dream cards.
        // 梦·大还丹 (D15064/5): each qualifying own-turn start, if my max_hp
        //   or hp is lower than enemy's, +N max_hp and +N hp.
        // 梦·飞云丹 (D15184/5): each own-turn start, if enemy has def then
        //   add N ignore_def stacks, else gain N qi.
        // Both magnitudes accumulate across copies; persist for whole battle
        // (mirrors elusive_footwork_stacks lifecycle — no per-turn decrement).
        this.dream_great_pill_stacks = 0;
        this.dream_flying_cloud_stacks = 0;
        // R32 — new state vars for 2026-05 patch cards. Pinged from hooks in
        // increase_idx_qi / reduce_idx_x_by_c / increase_idx_x_by_c (agility) /
        // increase_idx_activate / trigger_card / atk / is_crash_fist.
        this.bird_shadow_decree_stacks = 0;            // 百鸟曳影诀: on qi-loss → +sword_qi
        this.yu_ling_xin_fa_qi_gain_def_stacks = 0;    // 御灵心法 qi-gain branch → +def
        this.yu_ling_xin_fa_qi_loss_def_stacks = 0;    // 御灵心法 qi-loss branch → +def
        this.ji_jing_qi_xin_fa_stacks = 0;             // 极•静气心法: on qi-gain → +hp
        this.shan_zhuan_teng_nuo_stacks = 0;           // 闪转腾挪: on agility-gain → +def (passive)
        this.feng_xu_yu_feng_stacks = 0;               // 冯虚御风: on agility-gain → enemy dmg
        this.zui_quan_jia_shi_stacks = 0;              // 醉拳架势: on flaw-loss → +increase_atk
        this.zhen_yin_xin_fa_stacks = 0;               // 镇印心法: on element-activate → enemy dmg
        this.ji_yun_jian_rou_xin_stacks = 0;           // 极•云剑柔心: on cloud_sword played → +hp
        this.ji_shui_ling_zhen_stacks = 0;             // 极•水灵阵: on water_spirit played → +qi
        this.shang_hun_zhou_zhen_stacks = 0;           // 伤魂咒阵: on atk OR take-atk → enemy injury (decremented)
        this.ji_beng_tian_bu_stacks = 0;               // 极•崩天步: all cards count as 崩拳
        this.hp_cost_cards_used = 0;                   // counter — # hp_cost cards played this battle (for 万魂破军)
        // R33 — hp_cost-triggered carry stacks. Pinged in hp_cost handler.
        this.crash_fist_shattered_bone_carry_stacks = 0;  // 崩拳•碎骨: next 崩拳's hp_cost → enemy dmg
        this.blood_shadow_escape_stacks = 0;              // 血影遁术: next hp_cost card's hp paid → +agility
...
export class GameState {
    ...
    do_cloud_sword_chain_count(card_id) {
        ...
        if (this.is_cloud_sword(card_id)) {
            ...
            this.reduce_idx_x_by_c(0, "m_light_sword_stacks", 1);
        } else {
            if (me.cloud_sea > 0) {
                me.cloud_sea -= 1;
                me.cloud_sword_chain_count += 1;
    ...
    do_dream_unrestrained_sword_zero_setup() {
        ...
        if (count > 0) {
            this.increase_idx_x_by_c(0, "dream_unrestrained_sword_zero_stacks", count * 2);
        }
    }
    ...
    is_crash_fist(card_id) {
        return (
            ...
            this.players[0].ji_beng_tian_bu_stacks > 0 ||  // R32: 极•崩天步
        is_crash_fist(card_id));
    }
    ...
    trigger_card(card_id, idx) {
        ...
        // R32 on-card-played hooks. Trigger BEFORE the card body runs so the
        // qi/hp bonus is available to the played card itself.
    ...
    play_card(card_id, idx) {
        ...
        for (let i=0; i<plays; i++) {
            ...
            if (this.is_sword_formation(card_id)) {
                me.played_sword_formation_count += 1;
            }
            if (this.is_sword(card_id)) {
                me.played_sword_count += 1;
            }
        }
    ...
    sim_turn() {
        ...
        // R22: 梦·大还丹 — turn-start, if losing in max_hp OR hp, gain N of both.
        //   Slash in game text "生命上限/生命小于对方" reads as alternative
        //   ("or"); the +N applies to both stats when condition fires.
        if (me.dream_great_pill_stacks > 0
            && (me.max_hp < enemy.max_hp || me.hp < enemy.hp)) {
            this.increase_idx_max_hp(0, me.dream_great_pill_stacks);
            this.increase_idx_hp(0, me.dream_great_pill_stacks);
        }
        // R22: 梦·飞云丹 — turn-start branch: if enemy has def, gain ignore_def
        //   stacks (next-N-attacks ignore defense); else gain qi.
            ...
            if (me.qi < qi_cost) {
                ...
                if (hp_cost !== undefined) {
                    if (hp_cost > 0) {
                        ...
                        // R32 counter: # hp_cost cards played this battle.
                        // 万魂破军 reads this for its [气势] scaling.
                        me.hp_cost_cards_used += 1;
                        // R33 hp_cost-paid hooks. Run AFTER the cost is paid so
                        // stacks set by THIS card's own card_actions only fire
                        // on the NEXT hp_cost card (carry semantics).
    ...
    increase_idx_activate(idx, x, amt) {
        ...
        // R32 element-activate hook (镇印心法)
    ...
    deal_damage_inner(dmg, is_atk, my_idx, is_extra) {
        ...
        if (enemy.dream_repel_citta_dharma_stacks > 0) {
            dream_repel_reflect = Math.ceil(dmg * 30 / 100);
    ...
    atk(dmg, is_extra) {
        ...
        // R32: 伤魂咒阵 — same shape as 摘花飞叶 but stacks DECREMENT per trigger
        // (the "[持续]N次" semantics). Applies on attack from either side.
    ...
    do_five_elements_pure_vase() {
        ...
        for (let i=0; i<me.five_elements_pure_vase_cards.length; i++) {
            ...
            if (swogi[card_id].name === "Forget Worries") {
                this.increase_idx_max_hp(0, 15);
                this.increase_idx_hp(0, 15);
                this.increase_idx_max_hp(1, 15);
                this.increase_idx_hp(1, 15);
            }
        }
    }
