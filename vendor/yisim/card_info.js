...
export function make_card_name_to_id_fuzzy(fuzzy) {
    return function card_name_to_id_fuzzy(name) {
        ...
        if (card_name_to_id[name] !== undefined) {
            return card_name_to_id[name];
        }
        const searchResult = fuzzy.search(card_names, name);
...
export const ready = (async () => {
    ...
    const PREFIX_TO_MARKING = {
        ...
        ["15"]: "no_marking", // extension prefix used by this repo for dream cards yisim doesn't ship (Round 18.8)
        ["16"]: "no_marking", // R30 extension prefix: 2026-05 patch's new cards (灵羽/极•X family). User flagged class assignment as unreliable so we don't categorize — leave as no_marking until proven.
